...
| # | Sev | Defect | Anchor |
|---|-----|--------|--------|
| B1 | Blocker | 🟢 **Fixed, pending closure-v3 re-review** (`atomic effect outbox` → `closure-v2` → `closure-v3`) — atomic state+effect write + at-least-once drain; `request_tier2` is a durable job (single-flight by record id, ACK only after durable reflection or stale-cancel, teardown→retry). Identity is an **opaque monotonic `requestId`** (reducer stamps `pendingTier2.requestId` from `GaugeState.tier2ReqSeq`; flows pending→outbox→token→`tier2_cancel`), so same-millisecond same-page/reason requests are distinguishable — an old job can't cancel/apply to a newer one. Apply guard also requires the same durable goal `epoch` (monotonic across clear). Writer text durable, dropped in the outbox tx; resetState serialized + one-transaction; **DB v3 migration wipes legacy wedged pending/outbox**; `setGoal` epoch bump serialized. _Note: reducer/types now diverge from the Python core (added `tier2ReqSeq`/`requestId`, `tier2_cancel`) — intended, server deleted at cutover._ Tests: requestId instance match, migration, epoch linearization, ack/keep/CAS, reset atomicity. | `src/core/gauge/*`, `src/lib/gaugeRuntime.ts`, `src/lib/tier2Token.ts`, `src/lib/session.ts`, `src/lib/db.ts` |
| B2 | Blocker | 🟡 **Guard laid, not yet closed.** The page/goal-revision **token** is now enforced at the Tier-2 apply (`dispatchTier2`): a result only applies while the judged page is still active + goal unchanged, else `tier2_cancel`. Remaining B2 work: the same guard for the Tier-0/1 `nav` path in `background.ts` (a slow embed dispatched against a page the user already left) and a decision on whether to also harden the reducer. | `src/background.ts:75`, `src/core/gauge/reducer.ts` |
| B3 | Blocker | ✅ **CLOSED** (`durable dwell timer` + closure + `closure-v2`) — `DwellScheduler`: checkpoint deleted only after a successful judge and only via CAS (never clobbers a newer dwell); reconcile single-flight; idle / focus-loss / goal-change cancel the dwell. A new candidate **atomically replaces** the checkpoint (single write) — no cancel→schedule gap where a teardown loses it. fake-indexeddb tests: restart-restore, stale-cancel, mid-judge retry, duplicate single-flight, CAS-no-clobber, atomic-replace. **Test caveat:** exercised against real IndexedDB via the extracted units (like the original's `PersistentDwellScheduler`), not a full `background.ts` chrome-mock harness — that E2E harness is deferred to the B4/B5 → cutover window. | `src/lib/dwellScheduler.ts`, `src/background.ts` |
