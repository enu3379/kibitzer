...
| B2 | Blocker | ✅ **CLOSED** (closure-v5). `judgeAndDispatch` now re-checks after its awaits, before any mutation, that the **active tab is still the judged page AND the goal epoch is unchanged** (`stillJudging`); else it drops the verdict — no stale apply to a page the user left / a changed goal. The v4 `serializeJudge` was removed (its role — stopping out-of-order active-page pinning — is subsumed by the stale-drop, and it caused head-of-line blocking). | `src/background.ts` (`judgeAndDispatch`, `stillJudging`) |
...
| B9 | High | ✅ **CLOSED** (B9 commit). Negative/0/non-finite time budget rejected at three layers (tBudgetSeconds fallback — the load-bearing NaN guard — plus popup + set-goal). Snooze-suppressed S=0: the gate now re-fires for a page pinned at 0/DRIFT that was never nudged once snooze ends. Tests added. | `src/core/gauge/config.ts`, `src/core/gauge/reducer.ts`, `src/popup/popup.ts`, `src/background.ts` |
