...
| # | Sev | Defect | Anchor |
|---|-----|--------|--------|
| B1 | Blocker | ✅ **CLOSED** (`atomic effect outbox` + `close B1/B3 residual defects` + `closure-v2`) — atomic state+effect write + at-least-once drain; `request_tier2` is a durable job (single-flight by record id, ACK only after durable reflection or stale-cancel, teardown→retry). Outcome applied guarded (serialized) on a **request-instance token** (page + reason + the slot's `requestedAt` + durable goal `epoch`), so an old job can't apply to / cancel a newer same-page/reason request or a redeclared-goal session; stale → additive `tier2_cancel` (no wrong-page side effect). Writer text in durable kv, its key dropped in the SAME tx as the outbox append. resetState serialized + one-transaction. Tests: token instance match, epoch monotonicity, ack/keep/CAS, reset atomicity. | `src/lib/gaugeRuntime.ts`, `src/lib/tier2Token.ts`, `src/lib/session.ts`, `src/lib/db.ts` |
