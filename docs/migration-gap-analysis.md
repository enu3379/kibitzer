...
| # | Sev | Defect | Anchor |
|---|-----|--------|--------|
| B1 | Blocker | 🟢 **Fixed, pending re-review** (`atomic effect outbox` → `closure-v2` → `closure-v3` → `closure-v4`). Durable Tier-2 job (single-flight, ACK only after durable reflection/stale-cancel, teardown→retry). Identity = opaque monotonic **`requestId`** carried **per-effect** (each request_tier2 effect stamps its own id — a promotion+s_zero in one reduce no longer collide) and **preserved across resetState** (never reused, so a zombie job can't cancel/apply to a fresh request). Apply guard also requires the durable goal `epoch`. Writer text durable + dropped in the outbox tx + cleared if unconsumed; resetState serialized + one-tx; **DB v3 migration is surgical** (releases the wedge, PRESERVES gauge progress); `setGoal` epoch serialized; DB `onblocked` diagnostic. _Reducer/types diverge from the Python core (`tier2ReqSeq`/`requestId`/`tier2_cancel`) — intended, server deleted at cutover._ Found by a 3-lens adversarial self-review; tests: per-effect requestId, migration-preserves-session, epoch linearization, ack/keep/CAS. | `src/core/gauge/*`, `src/lib/gaugeRuntime.ts`, `src/lib/tier2Token.ts`, `src/lib/session.ts`, `src/lib/db.ts` |
| B2 | Blocker | 🟡 **Guard laid, not yet closed** (adversarial review refined it). The instance token (requestId + epoch) is enforced at the Tier-2 apply. `judgeAndDispatch` is now **serialized** (closure-v4) so concurrent judges can't pin the active page to an older one or lose a read-modify-write — but it still does NOT re-check the active-page / goal-epoch **after** its awaits, so a Tier-0/1 verdict for a page the user already left (or a goal changed mid-embed) still applies. That re-check across each await is the remaining B2 work. | `src/background.ts` (`judgeAndDispatch`), `src/core/gauge/reducer.ts` |
