...
| # | Sev | Defect | Anchor |
|---|-----|--------|--------|
| B1 | Blocker | ✅ **CLOSED** (`fix: atomic effect outbox …` + `fix: close B1/B3 residual defects …`) — atomic state+effect write + at-least-once drain, and `request_tier2` is now a durable job: single-flight by record id, serviced off the queue, ACKed only after the outcome is durably reflected or stale-cancelled (teardown → retry on next drain/startup). Outcome applied guarded (serialized): only if pending slot (page+reason) + goal revision + active page still match, else released via the additive `tier2_cancel` event with no wrong-page side effect. Writer nag text moved to a durable kv bound to the nag record (global dropped). fake-indexeddb tests for ack/keep/CAS. | `src/lib/gaugeRuntime.ts`, `src/lib/db.ts` |
...
| B3 | Blocker | ✅ **CLOSED** (`fix: durable dwell timer …` + closure) — extracted `DwellScheduler`: checkpoint deleted only after a successful judge and only via compare-and-delete (never clobbers a newer dwell); reconcile is single-flight; idle / focus-loss cancel the dwell. fake-indexeddb integration tests: restart-restore, stale-cancel, mid-judge retry, duplicate single-flight, CAS-no-clobber. **Test caveat:** these exercise the extracted units against real IndexedDB (like the original's `PersistentDwellScheduler`), not a full `background.ts` chrome-mock harness — porting that end-to-end harness remains open. | `src/lib/dwellScheduler.ts`, `src/background.ts` |
