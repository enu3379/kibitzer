...
UX/analysis/context follow-ups, not claims of complete feature parity. They are
tracked by #135, #136, and #141 and remain visible in this table.
...
| B2 | Blocker | ✅ **CLOSED** (closure-v5 + **v6** + **neutral-dwell**). `judgeAndDispatch` re-checks after its awaits, before any mutation, that the **active tab is still the judged page AND the goal epoch is unchanged** (`stillJudging`); else drops the verdict. v4 `serializeJudge` removed (stale-drop subsumes it, no head-of-line block). **v6:** re-review found the drop left the debounce marker (`lastObservedKey`) on the never-judged page, so it was never re-judged on return — now cleared on drop; and the removed serialization's concurrent-RMW loss (recent-titles/anchor/exemplar) is closed via an atomic `kvUpdate`. **neutral-dwell (real-log 2026-07-24, `bd869b2`):** dropping a stale verdict still left the gauge *holding* the previous page's DRIFT, so S kept draining while the user read on-goal pages whose per-article judges were dropped (the "late reflection" symptom in exported logs) — now the gauge goes NEUTRAL (freezes) the moment a new page is observed, until the fresh judgement lands. See "Real-log refinement" below. | `src/background.ts`, `src/core/gauge/{types,reducer}.ts`, `src/lib/{gaugeRuntime,relevance,history,db}.ts` |
...
| B6 | High | 🟡 **Mostly closed** (B6 commit). Durable display token in the SSOT (no more SW-restart id reuse → markNagActed acts on the right nag); OS-notification buttons carry the `displayToken`; body-click "accepted" is handled by the generic feedback path. **Remaining (UI parity, not correctness; #141):** an explicit current-page verdict card / DRIFT label / pending-intervention surface in the popup. | `src/lib/gaugeRuntime.ts`, `src/background.ts` |
| B7 | High | 🟡 **Mostly closed** (B7 + **closure-v7**). Replay is **presence-aware**: `replayGauge` treats an away minute as an inactive rebase (no drain), so a 39-min AFK no longer replays as active drift; old logs fall back to assume-present; presence sorted defensively. **closure-v7** (cutover re-review) reworked the presence LOGGING: it now logs only from the heartbeat using `browserPresent()` (all sources agree — fixed an "active in another app" phantom-drain) and dedups against a DURABLE last-state (was a per-minute redundant log that could evict observes). **Remaining (lower value):** replay re-thresholds recorded scores; it doesn't re-run exemplar/anchor/enrichment learning or the feedback timeline. | `src/lib/replay.ts`, `src/background.ts` |
| B8 | High | 🟡 **Mostly closed** (B8 commit). Tier-1 rescue now receives the recent-visit titles; Tier-2 reports the observe-time `tier_reached` instead of a hardcoded 0 — 1 only when Tier-1 actually returned a verdict (the B8-era "reaching the gate implies Tier-1 ran" shortcut stopped holding once the tiers routed to providers independently; corrected with #207). **Remaining (needs extra persistence/privacy call):** the cross-lingual derived-phrase TEXT and recent-PAGE-content aren't stored, so those two payload fields stay empty. | `src/lib/tier12.ts`, `src/background.ts` |
...
| B10 | High | 🟡 **FOLLOW-UP (#135/#141).** Session end + end-summary landed (P3-8, feature/session-summary); pause, daily & session reports, navigation history, and verdict correction remain unimplemented. The strategic call — accepted as post-cutover UX parity work, not a correctness/privacy deletion gate — lives in [planning-notes.md](planning-notes.md) **D14**. | `docs/migration-gap-analysis.md` P3-3 |
...
### Real-log refinement (2026-07-24) — neutral-dwell hold + score↔page logging (`bd869b2`)

Analysis of an exported real session (`kibitzer-events.json` + `kibitzer-debug.txt`, goal "네이버 뉴스보기") showed S draining through a long DRIFT stretch punctuated by three `judge dropped (page/goal moved on)` lines on **on-goal news pages**: the user was reading news, but each article's judge was dropped (B2 `stillJudging` + B4 per-article `pageKey`), so the gauge kept integrating the **previous off-goal page's DRIFT** — S bled to near-zero while the user was on-goal ("detection feels off / reflected late"). The debug log also couldn't tie a score change to the page that caused it (heartbeat lines carried S but no page).

Fix:

- **Neutral hold through the dwell** — a new `neutral` gauge event integrates the page just left up to the navigation instant (its drift still counts; the S=0 gate can still fire), then drops the verdict (`activeVerdict=null` → `advance()` early-returns). S and m **freeze** — no drain, no recover — until the dwell's `nav` supplies the fresh verdict, at which point integration resumes all at once from the judge time (the frozen interval is not back-integrated). `enterNeutral` no-ops when already neutral or still on the same page (title churn / SPA storms don't disturb a live verdict). Sensitive pages go neutral too, so a prior DRIFT can't drain across heartbeats spent on a page we won't judge. _`neutral` is a wiring-only reducer event like `tier2_cancel` — the shared parity fixtures never emit it, so the golden/property fixtures still pass; the divergence from the Python core is intended._
- **Score↔page logging** — klog gains `page=<basis>` + a `NEUTRAL` label; a durable `tick` event records `{event, s, m, verdict, page, prevVerdict, prevPage, effects}` so offline replay can reconstruct the S curve against the active page. (`observe`/`tick` stay distinct event types → `replay.ts` unaffected.)
- **Drift-episode clock** guarded so a neutral hold between two off-goal pages doesn't reset persona `drift_minutes` on every navigation; cleared on full recovery (S→100 on OK) so the next drift is timed from its own start.
- **Tests:** `core/gauge/neutral.test.ts` (integrate-to-nav / freeze / resume-from-nav) **plus a third `background.ts` E2E scenario** — drift on a page → navigate to a new (unjudged) page → S held flat across 15 heartbeats, never drains to 0, no nag — which also begins to close B3's "no full background E2E harness" caveat.
...
| P3-3 | **Self-focus analytics: session stats / report / history** | FOLLOW-UP (#135/#141) | L | "How did my focus go today" — focus ratio, hourly strip, top drift hosts, longest OK stretch. On a **separate page**. Aggregates the `events`/`observations` stores. |
...
| P3-8 | Session pause + end summary | 🟡 **END-SUMMARY DONE** (pause still open, #141) | S | 종료하기 now shows a session summary in the popup: valid-page ratio + valid dwell time (band-colored HP-bar meters), nag count, longest-dwell page, and an async Tier-2 persona recap with style dice (`lib/visits.ts` runtime tracker, `lib/sessionStats.ts`, `lib/sessionSummary.ts`, `lib/summaryDice.ts`). Pause remains: a `paused` flag short-circuiting observe/heartbeat. |
