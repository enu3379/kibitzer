...
후속 반영(2026-07-16): MiniMax 로컬 프로필은 Writer 2048로 올렸고, Ollama의
`eval_count >= writer_max_output_tokens` 및 OpenAI 호환 응답의
`finish_reason=length`는 Writer 실패로 처리하도록 제품 transport와 이 감사
스크립트를 갱신했다. 빈 응답 재시도와 `think:"low"`는 지연·호환성 실험 전까지
보류한다.
