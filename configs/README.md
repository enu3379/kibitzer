...
- `personas.yaml` + `personas/` - built-in persona voice data (voices, templates,
  ...
  `apps/extension-next/src/lib/personas.data.ts`.
- `sensitive_domains.json` - domain block/drop rules imported at build time by the
  extension's domain filter (`apps/extension-next/src/lib/domainFilter.ts`).
...
changes, the Tier 0 `tau` in `apps/extension-next/src/lib/tier0.ts`, the relevance
floors in `apps/extension-next/src/lib/relevance.ts`, exemplars, and anchor
vectors must be recalibrated. The recalibration study lives in
`apps/extension-next/tools/anchorFloorStudy.ts`, with results recorded in
