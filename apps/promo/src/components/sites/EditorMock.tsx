...
/**
 * The document surface. `lines` are the outline items typed live; `body` is the prose
 * that exists once the report is further along.
 */
export const EditorMock: React.FC<{
  ...
  scroll?: number;
  complete?: boolean;
}> = ({ lines, showCaret = false, body = [], scroll = 0, complete = false }) => (
  <div style={{ position: "absolute", inset: 0, background: "#eceded", overflow: "hidden" }}>
    ...
    <div style={{ position: "absolute", top: 34, left: 0, right: 0, bottom: 0, overflow: "hidden" }}>
      ...
      >
        ...
        <div style={{ fontSize: 11.5, color: "#80868b", marginBottom: 22 }}>{report.subtitle}</div>

        {lines.map((l, i) => (
          <div key={i} style={{ fontSize: 13.5, lineHeight: 1.9, color: "#202124", fontWeight: 600, wordBreak: "keep-all" }}>
            {l}
            {showCaret && i === lines.length - 1 ? <Caret /> : null}
          </div>
        ))}
          ...
          <div style={{ marginTop: 16 }}>
            {body.map((p, i) =>
              p === "" ? (
                ...
                <div key={i} style={{ fontSize: 12.5, lineHeight: 1.95, color: "#3c4043", wordBreak: "keep-all" }}>
                  {p}
                </div>
              ),
            )}
        ...
        {complete ? (
          <>
            ...
            <div style={{ fontSize: 10.5, color: "#80868b", marginTop: 7 }}>[표 1] 유인 수단별 획득 비용 대비 90일 잔존율</div>
          </>
        ) : null}
      </div>
    </div>
