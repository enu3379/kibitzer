...
export function showKibitzerToast(payload: ToastPayload): void {
  ...
  shadow.innerHTML = `
    <style>
      ...
      .card { position: relative; background: ${surface}; border: 1.5px solid #10B981; border-radius: 12px; padding: 12px 14px; box-shadow: 0 6px 24px rgba(0,0,0,.18); cursor: pointer; }
  ...
  // Clicking the bubble itself = "잘 잡았어요", mirroring the system notification body click.
  shadow.querySelector(".card")?.addEventListener("click", () => settle("accepted"))
